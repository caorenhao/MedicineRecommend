...
public class FileUtil {
     ...
     *
     * @param file 待切分的指定文件
     * @param outputPath 切分后小文件的保存路径
     * @param start 文件起始编号
     * @return int 返回创建了多少个小文件
     * @throws IOException 
     */
    public static int split(File file, String outputPath, int start) throws IOException {
        ...
        int number = (int)Math.floor(file.length()/bufferSize);
        int splitNum = number-1;//要分割的块数减一
        int fileLines = FileUtil.getLineNumber(file);//输入文件的行数
        long perSplitLines = fileLines / splitNum;//每个块的行数
        System.out.println("文件长度为:" + file.length());
        System.out.println("文件分为:" + number + "个小文件");
        System.out.println("每个小文件的行数为:" + perSplitLines);
        for (int i = (0+start); i <= (splitNum+start); ++i) {
            //每个块建立一个输出
            FileWriter fw = new FileWriter(outputPath + i + ".txt");  
            String line = null;
            //逐行读取，逐行输出
